...
このファイルは、`packages/visualization` 配下で作業する agent 向けのガイドです。

## Package Role

`visualization` は、図解とドキュメント視覚化ツールをまとめる dependency-only aggregator です。[apm.yml](./apm.yml) を正本として扱ってください。

対象は、visual design 能力に絞ります。図解生成、architecture 視覚化、ドキュメント形式の標準化が主な範囲です。
