...
`priority`, then stable edge ID; one edge is returned per turn. The caller runs
all returned audits, dispatches only the returned delegate, records Markdown
evidence, and re-enters at `next`.
