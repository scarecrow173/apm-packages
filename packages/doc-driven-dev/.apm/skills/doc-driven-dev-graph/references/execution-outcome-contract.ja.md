...
| `spec`, `adr`, `design`, `plan`, `task`, `test-spec`, `impl-record`, `all` | named audit 用の caller adapter |
...
は選択された Task Graph task document、`test-spec` は対応する test specification document、
`impl-record` はその Implementation Record、`all` は
