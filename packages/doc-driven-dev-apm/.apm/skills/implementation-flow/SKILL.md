...
> `sdp scan`, agent review/update of inference entries, `sdp infer`, and
> `sdp profile`, then updated when skills change.

- If it exists and is valid → go to Phase B (Configuration from Profile).
- If it does not exist → run `sdp scan --adapter .apm/skills/implementation-flow/assets/adapters/implementation-adapter.yaml` then `sdp infer init --scan .sdp/skill-scan-list.json` then `sdp profile --adapter .apm/skills/implementation-flow/assets/adapters/implementation-adapter.yaml`
- If it exists but is corrupted → run the same `scan` → `infer` → `profile` sequence.
