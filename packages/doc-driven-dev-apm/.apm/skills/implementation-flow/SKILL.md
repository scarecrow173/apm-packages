...
| ----- | ------- | ------ |
| A. Assess | Understand task, check for `implementation-profile.json` | Task characteristics identified |
...
This mapping is a general framework. The invocation resolution in `implementation-profile.json` is
the repository-specific instantiation of this mapping, specifying concrete skill names and conditions.

**Check for Profile:** Check for `implementation-profile.json` at repository root.

> **What is `implementation-profile.json`?**
...
With `implementation-profile.json` available:

1. **Load flow stack**: `sdp query --profile implementation-profile.json flow-stack`
2. **Check resolution**: `sdp query --profile implementation-profile.json resolution`
3. **Check execution policy**: `sdp query --profile implementation-profile.json execution-policy --skill <name>`
...
2. Check the skill's **execution mode** from the profile:
   ...
   Use `sdp query --profile implementation-profile.json execution-policy --skill <name>` to check.
