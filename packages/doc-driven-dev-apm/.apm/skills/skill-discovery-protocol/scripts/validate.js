  ...
  "src/skills/skill-discovery-protocol/scripts/lib/gates/schema_gate.ts"(exports2, module2) {
    ...
    function validateProfileSchema(profile) {
      ...
      if (profile.resolved_invocations && Array.isArray(profile.resolved_invocations)) {
        for (let i = 0; i < profile.resolved_invocations.length; i++) {
          const inv = profile.resolved_invocations[i];
      ...
      if (catalog.skills && Array.isArray(catalog.skills)) {
        for (let i = 0; i < catalog.skills.length; i++) {
          ...
          if (!skill.execution_policy || typeof skill.execution_policy !== "object") {
            ...
            const ep = skill.execution_policy;
            if (!["rigid", "flexible"].includes(ep.strictness)) {
    ...
    function validateAdapterSchema(adapter) {
      ...
      if (adapter.flow_stack && typeof adapter.flow_stack === "object") {
        const fs2 = adapter.flow_stack;
        if (!Array.isArray(fs2.slots)) {
          errors.push({ field: "flow_stack.slots", message: "flow_stack.slots must be an array" });
        } else {
          for (let i = 0; i < fs2.slots.length; i++) {
            const slot = fs2.slots[i];
            if (!slot.slot_id || !isSnakeCase(slot.slot_id)) {
