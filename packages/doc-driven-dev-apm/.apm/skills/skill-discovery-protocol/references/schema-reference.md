# Skill Discovery Protocol — Schema Reference

Version: 1.0.0

This document describes the Adapter YAML schema, invocation resolution
rules, scan aggregation, classification validation, and execution policy.

For CLI commands, see `cli-reference.md`.
For operational rules and constraints, see `operation-policy.md`.

---

## 1. Adapter YAML Schema
...
### 2.1 `resolved_invocations` Generation
...
`runtime_guidance` is not part of `resolved_invocations`. It is a separate
structured metadata channel that flow reads after `execution_policy` has ruled
out incompatible candidates.
...
## 3. `scan.scopes` Aggregation Rules
...
### 4.2 Unmatched Policy Validation
...
### 5.1 Skill Reference Catalog `execution_policy`
...
`runtime_guidance` is flow-specific structured metadata that is stored in the
Flow Profile, not in the catalog. The flow reads it after `execution_policy`
...
| ----- | ---- | ----------- |
| `priority_delta` | number | Relative priority adjustment for a candidate |
| `prefer_when` | string[] | Conditions that should increase preference |
| `avoid_when` | string[] | Conditions that should decrease preference |
| `requires_sequence` | boolean | Indicates sequence dependence |
| `requires_step_reordering` | boolean | Indicates step reordering is required |
| `requires_partial_application` | boolean | Indicates partial application is required |

`runtime_guidance` is a soft ranking signal, not a hard constraint.
