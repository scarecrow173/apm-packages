...
既定では、判断が単純で、誤解が起きにくく、保存すべき重要なトレードオフが
少ない場合に限って `minimal` を使います。
...
| `full` | 複数の実質的な選択肢、明示すべき decision drivers、重要なトレードオフ、大きな影響、複数チームへの影響、リスク、コンプライアンス観点、または将来の再レビューがある場合。 |
| `bare-minimal` | リポジトリ側に厳密な ADR 文言規約があり、MADR の最小セクション構造だけを挿入したい場合。 |
| `bare` | リポジトリ側に厳密な ADR 文言規約がありつつ、MADR の完全なセクション構造が有用な場合。 |

ステークホルダーが結論だけでなく判断過程を見る必要がある場合は `full` を
優先します。また、後から判断を見直す可能性があり、選択肢間の比較を残す
必要がある場合も `full` を優先します。
