...
**Check for Profile:** Check for `briefing-profile.md` at repository root.

> **What is `briefing-profile.md`?**
> A repository-specific configuration file that lists all skills available for
> briefing, assigns them to categories, defines which are always-on vs conditional,
> and specifies the default skill stack plus override rules. Generated once by the
> Skill Discovery Protocol and updated when skills change.
...
- If it exists but is corrupted → delete and regenerate via Skill Discovery Protocol.

---

## Skill Discovery Protocol

The protocol runs when `briefing-profile.md` does not exist or is detected as stale.
...
2. **Classify** — Assign each to a Briefing category (Discover, Frame, Research, Validate, Document).
...
With `briefing-profile.md` available:

1. **Load the Default Stack** — these skills apply to every briefing.
2. **Apply Override Rules** — check information characteristics against override conditions.
3. **Resolve conflicts** — if multiple skills in the same category are activated:
   - More specific condition wins over general.
   - Explicit profile rule wins over inferred activation.
   - If still tied, apply both (skills layer, they don't exclude).
4. **Add skills based on Entry Decision** — prioritize categories matching Phase A path:
   - A-1/A-2 → Prioritize Frame-category skills.
   - A-3 → Activate matching skills across all categories.
   - A-5 → Prioritize Discover/Research-category skills.
   - **If no Override Rules match:** Proceed with Default Stack only.
