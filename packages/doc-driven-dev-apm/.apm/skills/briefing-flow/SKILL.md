...
> and specifies invocation resolution. Generated by `sdp scan` + `sdp infer` + `sdp profile` and validated
> by `sdp validate`.

- If it exists and is valid → go to Phase B (Configuration from Profile).
- If it does not exist → run: `sdp scan --adapter .apm/skills/briefing-flow/assets/adapters/briefing-adapter.yaml` then `sdp infer init --scan .sdp/skill-scan-list.json` then `sdp profile --adapter .apm/skills/briefing-flow/assets/adapters/briefing-adapter.yaml`
- If it exists but is stale/corrupted → run the same `scan` → `infer` → `profile` sequence
...
- If profile does not exist → run the `scan` → `infer` → `profile` sequence.
