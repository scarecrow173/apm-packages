...
test("deep-dive skill exists in both locales", () => {
  ...
  const ja = fs.readFileSync(path.resolve(__dirname, "../../../packages/doc-driven-dev/.apm/skills/deep-dive/SKILL.ja.md"), "utf8");
...
test("adr-doc keeps core workflow while delegating deeper clarification", () => {
  const adr = fs.readFileSync(path.resolve(__dirname, "../../../packages/doc-driven-dev/.apm/skills/adr-doc/SKILL.md"), "utf8");
