...
function renderGraph(snapshot: DashboardSnapshot): string {
  ...
  const gateRows = gateEntries.map(([name, gate]) => [e(name), e(gate.status), list(gate.reasons)]);
  ...
  const count = (n: number) => `<span class="lane-count" aria-label="${n} 件">${n}</span>`;
  return `<section id="graph" aria-labelledby="graph-heading"><h2 id="graph-heading">Graph</h2><div class="probe-grid">${routeSummary}<div class="probe-side">${signalPanel}${gatePanel}</div></div>${canvas}<details><summary>全 graph node ${count(snapshot.definition.nodes.length)}</summary>${table(["node ID", "kind", "delegate", "audits", "commit gate", "terminal", "reachable"], nodeRows, "node 0 件")}</details><details><summary>全 graph edge ${count(snapshot.definition.edges.length)}</summary>${table(["edge ID", "from", "to", "condition", "priority"], edgeRows)}</details><details><summary>gate ${count(gateRows.length)}</summary>${table(["gate", "status", "reasons"], gateRows, "gate 0 件")}</details><details><summary>Graph topology issues ${count(topologyRows.length)}</summary>${table(["severity", "code", "node", "condition"], topologyRows)}</details></section>`;
}
