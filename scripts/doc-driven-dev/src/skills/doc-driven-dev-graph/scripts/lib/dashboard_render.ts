...
function renderBacklog(snapshot: DashboardSnapshot, ids: ReadonlyMap<string, string>): string {
  ...
  return `<section id="backlog" aria-labelledby="backlog-heading"><h2 id="backlog-heading">検討・着手候補（draft 文書）</h2><p>status が draft の canonical 文書です。今後レビュー・着手する対象として一覧します（${drafts.length} 件）。</p><div class="backlog-grid">${cards || `<p class="empty">draft 文書はありません</p>`}</div></section>`;
}
