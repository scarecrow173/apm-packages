...
function renderGraph(snapshot: DashboardSnapshot): string {
  ...
  const routeSummary = !decision
    ...
    : `<div class="callout route-probe"><h3>遷移プレビュー（指定条件からの評価）</h3><p class="route-path"><span class="chip chip-route">${e(route?.current)}</span><span class="route-arrow">→</span><span class="chip chip-route">${e(route?.next)}</span> ${statusPill(route?.status)}</p><dl class="facts"><div><dt>edge</dt><dd>${e(route?.edgeId)}</dd></div><div><dt>condition</dt><dd>${e(route?.condition)}</dd></div><div><dt>delegate</dt><dd>${e(route?.delegate)}</dd></div><div><dt>commit gate</dt><dd>${e(bool(route?.commitGate ?? false))}</dd></div></dl><p><strong>required audits:</strong> ${chips(route?.requiredAudits ?? [])}</p><p><strong>hard blockers:</strong> ${chips(explanation?.hardBlockers ?? [], true)}</p><p><strong>blocked reasons:</strong> ${chips(explanation?.blockedReasons ?? [], true)}</p></div>`;
