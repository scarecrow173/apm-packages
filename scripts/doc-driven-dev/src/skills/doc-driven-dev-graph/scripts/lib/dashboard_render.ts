...
export function renderDashboard(snapshot: DashboardSnapshot): string {
  ...
  return `<!doctype html><html lang="ja"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><meta http-equiv="Content-Security-Policy" content="default-src 'none'; style-src 'unsafe-inline'; script-src 'unsafe-inline'; img-src data:; base-uri 'none'; form-action 'none'; connect-src 'none'"><link rel="icon" href="data:image/svg+xml,%3Csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 16 16'%3E%3Crect width='16' height='16' rx='3' fill='%232563eb'/%3E%3C/svg%3E"><title>文書駆動開発の進行状況</title><style>${picoClasslessCss}${styles}</style></head><body><a class="skip-link" href="#main">本文へ移動</a><header><h1>文書駆動開発の進行状況</h1><p><strong>repository:</strong> ${e(snapshot.repositoryName)} / <strong>対象:</strong> ${list(snapshot.requested.focus)} / <strong>開始:</strong> ${e(snapshot.startedAt)} / <strong>生成時点:</strong> <time datetime="${escapeHtml(snapshot.generatedAt)}" data-relative>${e(snapshot.generatedAt)}</time></p></header><nav aria-label="セクション"><ul><li><a href="#attention">要対応</a></li><li><a href="#overview">概要</a></li><li><a href="#charts">チャート</a></li><li><a href="#task-board">タスクボード</a></li><li><a href="#backlog">候補</a></li><li><a href="#graph">Graph</a></li><li><a href="#tasks">タスク詳細</a></li><li><a href="#documents">文書</a></li><li><a href="#diagnostics">診断</a></li></ul></nav><main id="main">${renderAttention(snapshot)}${renderMetrics(snapshot)}${renderCharts(snapshot)}${renderTaskBoard(snapshot, ids)}${renderBacklog(snapshot, ids)}<div class="graph-scroll">${renderGraph(snapshot)}</div>${renderPlans(snapshot)}${renderDocuments(snapshot)}${renderDiagnostics(snapshot)}</main><footer><small>この画面は生成時点の状態です。更新するにはコマンドを再実行してください。</small></footer><noscript>全情報を表示しています。絞り込みには JavaScript が必要です。</noscript><script>${filterScript}</script></body></html>`;
}
